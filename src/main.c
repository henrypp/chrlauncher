...
)
{
	...
	status = _r_path_getfullpath (configured_directory->buffer, &full_path);

	if (NT_SUCCESS (status))
	{
		if (_r_fs_exists (&full_path->sr) && _r_fs_isdirectory (&full_path->sr))
			return full_path;
	...
	candidate = _r_format_string (L"%s\\..\\%s", app_directory->buffer, configured_directory->buffer);
	...
	status = _r_path_getfullpath (candidate->buffer, &full_path);
...
)
{
	...
	binary_dir = _r_config_getstringexpand (L"ChromePlusDirectory", L".\\chrome_plus");
