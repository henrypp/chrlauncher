...
VOID _app_prepare_addons_layout ()
{
	PR_STRING addons_config_path;
	...
	addons_directory = _r_format_string (L"%s\\%s", app_directory->buffer, APP_ADDONS_DIRECTORY);

	if (!addons_directory)
		return;

	if (!_r_fs_exists (&addons_directory->sr))
		SHCreateDirectoryExW (NULL, addons_directory->buffer, NULL);

	addons_config_path = _r_format_string (L"%s\\%s", addons_directory->buffer, APP_ADDONS_CONFIG_FILE);
	legacy_config_path = _r_format_string (L"%s\\%s.ini", app_directory->buffer, APP_NAME_SHORT);

	if (addons_config_path && legacy_config_path)
	{
		if (_r_fs_exists (&addons_config_path->sr))
		{
			_r_fs_copyfile (&addons_config_path->sr, &legacy_config_path->sr, FALSE);
		}
		else if (_r_fs_exists (&legacy_config_path->sr))
		{
			_r_fs_copyfile (&legacy_config_path->sr, &addons_config_path->sr, FALSE);
		}
	}

	if (addons_config_path)
		_r_obj_dereference (addons_config_path);
...
BOOLEAN _app_is_secure_dns_url (
	_In_ PR_STRING url
)
{
	if (_r_obj_isstringempty (url))
	...
	if (_wcsnicmp (url->buffer, L"https://", 8) != 0)
		return FALSE;

	return !!(StrStrIW (url->buffer, L"sky.rethinkdns.com") ||
		StrStrIW (url->buffer, L"/dns-query") ||
		StrStrIW (url->buffer, L"?dns") ||
		StrStrIW (url->buffer, L"/1:"));
}
...
)
{
	...
	if (!_r_fs_exists (&cache_directory->sr))
		SHCreateDirectoryExW (NULL, cache_directory->buffer, NULL);
...
PR_STRING _app_create_dns_blocklist_arguments ()
{
	...
	if (_app_is_secure_dns_url (blocklist_url))
	{
...
)
{
	...
	if (status == SZ_OK)
	{
		...
		if (!_r_obj_isstringempty (pbi->chrome_plus_dir) && _r_fs_exists (&pbi->chrome_plus_dir->sr) && _r_fs_isdirectory (&pbi->chrome_plus_dir->sr))
		{
